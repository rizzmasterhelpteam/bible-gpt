...
const LibraryScreen = ({ navigation }) => {
  ...
  const handleBookmark = async (verse) => {
    try {
      ...
      Alert.alert('Bookmarked! ✨', 'Verse saved to your bookmarks.');
    } catch (error) {
  ...
  const VerseCard = ({ verse, showBookInfo = false }) => (
    <View style={[styles.verseCard, { backgroundColor: theme.colors.verseBg }]}>
      ...
      <Text style={[styles.verseTextDisplay, { color: theme.colors.text }]}>{verse.text}</Text>
      <TouchableOpacity
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={styles.searchContainer}>
        <TextInput
          ...
          placeholder="Search for verses (e.g. peace, hope)..."
      ...
      <ScrollView style={styles.content}>
        {isLoading ? (
          ...
          <View style={styles.emptyState}>
            <Text style={styles.emptyIcon}>🔍</Text>
            <Text style={[styles.emptyText, { color: theme.colors.textSecondary }]}>
              {searchQuery ? 'No verses found. Try another keyword.' : 'Search for a keyword (e.g. love, peace, hope).'}
            </Text>
          </View>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  bookName: { fontSize: 15, fontWeight: '600', textAlign: 'center', marginBottom: 4 },
  bookInfo: { fontSize: 12 },
  chaptersGrid: {
    padding: 16,
    flexDirection: 'row',
    flexWrap: 'wrap',
  ...
  verseNumber: { fontSize: 13, fontWeight: 'bold', marginBottom: 8 },
  verseReference: { fontSize: 13, fontWeight: 'bold', marginBottom: 8 },
  verseTextDisplay: { fontSize: 16, lineHeight: 24, marginBottom: 12 },
  bookmarkButton: {
    paddingVertical: 8,
    paddingHorizontal: 16,
    borderRadius: 8,
  ...
  bookmarkButtonText: { color: '#FFFFFF', fontSize: 14, fontWeight: '600' },
  searchContainer: {
    flexDirection: 'row',
    padding: 16,
    gap: 8,
